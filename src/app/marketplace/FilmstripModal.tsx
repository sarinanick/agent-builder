...
export default function FilmstripModal({ item, navList, navIndex, onClose, onNavigate }: FilmstripModalProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('mousemove', filmMouseMove);
      window.removeEventListener('mouseup', filmMouseUp);
    };
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-zinc-900 rounded-2xl border border-zinc-800 flex overflow-hidden shadow-2xl w-full max-w-[1320px] h-[88vh] max-h-[820px]"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Filmstrip */}
          <div
            ref={filmstripRef}
            className="w-24 shrink-0 bg-black/40 overflow-y-auto p-2 flex flex-col gap-2 cursor-grab select-none max-md:hidden"
            onMouseDown={filmMouseDown}
          >
            {navList.map((navItem) => (
              <div
                key={navItem.id}
                id={`strip-${navItem.id}`}
                className={`relative rounded-lg overflow-hidden cursor-pointer shrink-0 border-2 transition-all ${
                  navItem.id === item.id
                    ? 'border-purple-500 opacity-100 shadow-lg shadow-purple-500/30'
                    : 'border-transparent opacity-50 hover:opacity-80'
                }`}
                onClick={() => !filmDrag.current.moved && onNavigate(navList.indexOf(navItem))}
              >
                <img src={`https://picsum.photos/seed/${navItem.seed}/120/120`} className="w-full h-16 object-cover" alt="" loading="lazy" />
              </div>
            ))}
          </div>
          ...
          {/* Image stage */}
          <div
            className="flex-1 bg-black flex items-center justify-center p-4 min-w-0 relative"
            onTouchStart={handleTouchStart}
            onTouchEnd={handleTouchEnd}
          >
            <img
              src={`https://picsum.photos/seed/${item.seed}/900/900`}
            ...
            <div className="absolute top-4 inset-x-4 flex items-center justify-between z-30 pointer-events-none">
              <span className="pointer-events-auto bg-black/60 text-white text-[11px] font-mono px-2.5 py-1.5 rounded-lg">
              ...
              <div className="pointer-events-auto flex items-center gap-2">
                ...
                <button className="bg-black/60 text-white text-[11px] px-3 py-1.5 rounded-lg hover:bg-black/80">
          ...
          <div className="overflow-y-auto p-5 md:p-6 flex flex-col gap-5 h-full w-full md:w-[340px] shrink-0">
            ...
            <div>
              ...
              <div className="bg-black border border-zinc-800 rounded-xl p-3 font-mono text-xs text-green-400 leading-relaxed">
                {item.prompt}
              </div>
            </div>
            ...
            <div className="mt-auto flex gap-2 pt-4 border-t border-white/5">
              <button className="flex-1 bg-gradient-to-r from-violet-600 to-pink-600 text-white font-bold py-2.5 rounded-xl text-sm hover:opacity-90 transition-opacity">
                Start Collaboration
              </button>
              <button className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold py-2.5 px-5 rounded-xl text-sm flex items-center gap-1.5 transition-colors">
                <Download className="w-3.5 h-3.5" /> 4K
              </button>
            </div>
              ...
              <div className="flex items-center gap-2">
                {item.team.slice(0, 4).map((member, idx) => (
                  <div key={idx} className="relative group/member">
                    <img src={`https://i.pravatar.cc/150?img=${member.img}`} className="w-9 h-9 rounded-full border-2 border-zinc-900" alt={member.name} />
                    <div className="opacity-0 group-hover/member:opacity-100 transition-opacity absolute bottom-full mb-2 right-0 bg-zinc-800 border border-zinc-700 rounded-xl p-3 w-44 shadow-xl text-right z-50 pointer-events-none">
                      <div className="flex items-center gap-2 mb-2">
                        <img src={`https://i.pravatar.cc/150?img=${member.img}`} className="w-9 h-9 rounded-full" alt="" />
                        <div>
                ...
                {item.team.length > 4 && (
                  <span className="w-9 h-9 rounded-full bg-zinc-800 border-2 border-zinc-900 flex items-center justify-center text-[10px] font-bold text-zinc-400">
                    +{item.team.length - 4}
                  </span>
                )}
              </div>
